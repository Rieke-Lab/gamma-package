classdef LedGammaMeasurement < edu.washington.riekelab.protocols.RiekeLabProtocol
    ...
    properties (Constant)
        numSteps = 10                  % Number of steps of intensity to measure
        zeroOffset = -0.0005            % Stating step intensity value
    end
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            if ~isfield(handler.userData, 'axesHandle')
                ...
                title(a, ['Optometer Power Measurement vs. ' obj.led ' Intensity']);
        ...
        function updateGammaTable(obj, handler, epoch)
            ...
            a = handler.userData.axesHandle;
            xlim(a, [min(obj.outputs(1:obj.currentStep)) - 0.05, max(obj.outputs(1:obj.currentStep)) + 0.05]);
            ylim(a, [min(obj.measurements) - 0.05, max(obj.measurements) + 0.05]);
